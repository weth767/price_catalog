...
@Service
public class SorensenDiceStrategyService implements OntologyStrategyService {
    @Override
    public Product compareSimilarity(List<Product> products, @NonNull String word) {
        List<Product> filteredProducts = products.stream()
                .map((product) -> {
                    double similarity = new info.debatty.java.stringsimilarity.SorensenDice().similarity(
